...
LAPD_divisions_geodata <- sf::read_sf("LAPD_Divisions_crimes.gpkg")

head(LAPD_divisions_geodata)

names(LAPD_divisions_geodata)

heat_map <- ggplot(data = LAPD_divisions_geodata) + 
  geom_sf(aes(fill = crimes_cou)) + 
  scale_fill_viridis_c(direction = -1, option = 'rocket') +
  labs(title = "Crime Choropleth Map of Los Angeles, USA",  # Adding a title
       fill = "Total of crimes", 
       caption = "Source: US Crime Department", 
